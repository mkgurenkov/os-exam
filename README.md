...
## 3-2. Эволюция операционных систем
...
Отказоустойчивость означает способность системы или компонента к продолжению нормальной работы, несмотря на наличие ошибок аппаратного или программного обеспечения.

Есть три основных показателя, связанные с отказоустойчивостью:
- <u>**надежность**</u> (reliability)
Надежность R(t) определяется как вероятность бессбойной работы системы до времени t, при условии ее нормально работы в момент времени t = 0.
- <u>**среднее время наработки на отказ**</u> (mean time to failure - MTTF)
Определяется как интеграл от R(t). По сути среднее время через которое система падает после полного восстановления.
- <u>**среднее время восстановления**</u> (mean time to repair - MTTR)
Среднее время полного восстановления системы.

![alt text](image-19.png)

<u>**Доступность**</u> системы - это доля времени, когда система доступна для обслуживания запросов пользователей:

![alt text](image-20.png)

По сути это вероятность того, что система работает.

Время, в течение которого система недоступна называется <u>**простоем**</u> (downtime), а время в течение которого доступна - <u>**временем безотказной работы**</u>.

### Отказы

Отказ - ситуация, когда система система оказывается в ошибочном состоянии по какой-то причине.

Можно сгруппировать отказы в следующие категории:
- постоянные
Такой сбой, который после того, как произошел, вызывает перманентное ошибочное состояние системы (или ее некорректную работу), которое не может быть устранено без устранения причины сбоя.
- временные
Сбой, при происхождении которого, ошибочное состояние (или некорректная работа) системы не сохраняется на все время ее эксплуатации.
  - однократные
  - периодические

###

В общем случае отказоустойчивость системы достигается путем внесения в нее избыточности. К методам резервирования относят следующие:
- пространственная (физическая) избыточность
Использование нескольких дублирующих компонентов (два процессора, два контроллера...). При этом, когда система функционирует "нормально" имеет смысл использовать все доступные компоненты, если есть такая возможность. Если какой-то из компонентов выходит из строя, система продолжает функционирование в деградированном режиме.
- временная избыточность
Просто повторить вычисление какое-то количество раз и сравнить результат. Обычно имеет смысл использовать совместо с информационной избыточностью (обнаруживается ошибка -> пересчитаем несколько раз).
- информационная избыточность
Всякие методы избыточного кодирования, код хэмминга, контольные суммы и тд.

ряд методов поддержки отказоустойчивости могут быть включены в программное обеспечение операционных систем. Некоторые примеры:
- изоляция процессов
Пользовательские процессы изолируются (а вот ядро кстати все в одном пространстве, именно поэтому ошибка в корявом драйвере может опрокинуть все ядро) как мы уже знаем, так что пользовательский процесс не убьет всю систему в случае сбоя.
- управление параллелизмом
Всякие дедлоки и лайфлоки могут коснуться и каких-то важных процессов, без которых система не будет работать полноценно и может уйти в отказ. Поэтому умение разрешать эти проблемы так же можно отнести к поддержке отказоустойчивости.
- виртуальные машины
С помощью виртуальной машины можно все-таки изолировать какой-нибудь драйвер вместе с приложением. Поэтому если он все-таки что-то сломает, то система не рухнет.
- точки восстановления и откаты
Если все таки система упадет или что-то пойдет не так, можно откатиться к какому-то стабильному состоянию.
...
- MIT, GE, Bell Labs: Multics, 1965 - непосредственный предок UNIX
- IBM: OS/360: Мейнфреймы System/360, 1966
- Technische Hogeschool Eindhoven: THE, Electrologica X8, 1968

**UNIX: BELL Labs, 1969, Кен Томпсон, Деннис Ритчи и Брайан Керниган**

Запущена в эксплуатацию в 1970 году на системе PDP-7.

Первым важным этапом стал перенос системы с PDP-7 на PDP-11, что послужило указанием на то, что UNIX может быть использована в качестве операционной системы на всех компьютерах.

Вторым важным этапом стала перепись ОС на язык программирования C (тогда считался высокоуровневым языком). Для того времени это было совсем неслыханно, чтобы операционная система писалась на языке "высокого" уровня.

Ну а дальше не особо интересно.

### Описание

- Ключевые понятия: вычислительный процесс и файл
- Компонентная архитектура: принцип "одна программа - одна функция"
Смысл в том, чтобы не писать программу как мультитул типа perf. Аналогия с cli: есть много разных утилит, которые можно объединять между собой разными методами (конвейер например) и удобно решать довольно сложные задачи. Тут про что речь интересно, реально про userspace утилиты (вроде как тоже считается частью ОС), или все таки про какую-то структуру ядра.
- Минимизация ядра
Тут мысль похожа как будто на микроядро. Лектор говорит что все остальные не основные функции вынесены "за ядро". То есть микроядро все таки?
- Независимость от аппаратной архитектуры и реализация на языке C
Реализация на языке высокого уровня позволило сделать ОС переносимой в каких-то пределах.
- Унификация файлов
...
Если кто-то разрабатывает ОС и называет ее UNIX, даже если чем-то она похожа на UNIX, но там будут несовместимые бинарные или прикладные интерфейсы, то по факту она UNIX являться не будет по идее. Короче решили как-то это дело регулировать и сделали Single UNIX Specification (SUS).

Поддерживается двумя компаниями: The Open Group и Austin Group. То есть нужно прийти к ним со своей ОС, они посмотрят и скажут юникс это или нет.

Эта сертификация (UNIX) включает в себя
- Основные определения
- Системные интерфейсы
- Командная оболочка и утилиты
- Пояснения
- X/Open Curses (графическая подсистема?)

Самые распространенные системы, которые могут называться юниксами: AIX, HP-UX, Solaris, MacOS

UNIX-like: Linux, FreeBSD, OpenBSD ...

#### POSIX

Unix-like это не юникс, но что-то что выглядит как юникс (может быть другое ядро, какие-то другие внутренние интерфейсы). Но тем не менее, такие операционные системы должны реализовывать стандарт POSIX (Portable Operating System Interface). 

Стандарт POSIX определяет минимальный интерфейс системных вызовов, который должны поддерживать совместимые с ним операционные системы.

## 3-8. Linux

### Описание

Линус Бенедикт Торвальдс, 1991 - ядро
Ричард Столлман, 1992 - библиотеки и обвзяка (GNU)

Первоначальная версия Linux была написана финским студентом Линусом Торвальдсом и была представлена в 1991 году. Линукс - бесплатная операционная система с открытым исходным кодом. Каждый может внести туда какие-то свои обновления/исправления, после чего они будут согласованы с руководителем подсистемы, а потом уже и с Линусом Торвальдсом.

GNU - GNU is Not Unix - это типа расшифровка (какая-то бредовая)

GNU - набор программ и инструментов, которые образуют большую часть пользовательской среды системы.
...
GNU/Linux — это операционная система, состоящая из ядра Linux и инструментов GNU.

**Дистрибутив** - берется какая-то версия ядра от Линуса Торвальдса, могут отличаться:
- библиотеками, утилитами (окружение)
- менеджером пакетов
- графическая подсистема
- прикладные программы
- поддержка (можно купить поддержку от чуваков, которые сделали этот дистрибутив и они будут консультировать, если вдруг возникнут какие-то ошибки/сложности)

Ubuntu, ArchLinux, Gentoo

Есть коммерческие и community версии дистрибутивов. Отличие в первую очередь как раз в наличии/отстутсвии поддержки ну и еще по мелочи: коммерческие версии более строго тестируются, поэтому они выходят реже, но зато стабильнее. Как правила приобретают для серверов.

### Основные компоненты ядра

У клименкова это основные подсистемы UNIX/LINUX.

Основные компоненты ядра операцационной системы (что подразумевается под компонентами не очень ясно, так что можно сказать просто - штуки которые есть в ОС):
- сигналы
Ядро использует сигналы для обращения к процессу
- системные вызовы
Системный вызов является средством, с помощью которого
процесс запрашивает определенную службу ядра. Существует несколько сотен
системных вызовов, которые можно грубо сгруппировать в такие категории: файловая
система, процессы, планирование, межпроцессное взаимодействие, сеть и др.
- процессы и планировщик
Создает, управляет и планирует процессы.
- виртуальная память
Выделяет виртуальную память для процессов и управляет ею.
- физическая память
Управляет пулом кадров страниц и выделяет страницы для
виртуальной памяти. Подсистемы виртуальной памяти и физической очевидно очень сильно связаны.
- файловая система
В достаточной степени виртуальная история. То есть подразумевается не только файлы на диске, но и любые другие. Файл оказался просто хорошей абстракцией много для чего.
- драйверы символьных устройств
Управление устройствами, которые требуют от ядра отправки или получения данных по одному байту (терминалы, принтеры и тд.).
- драйверы блочных устройств
Управление устройствами, которые читают/записывают данные блоками. Есть и символьный интерфейс: устройство все равно будет читать блоками, но ОС будет отдавать побайтово, в таком случае буфер устройства будет играть роль кеша. (диски, cd-rom, флешки и тд.).
- драйверы сетевых устройств
- сетевые протоколы
- ловушки и отказы
Обработка прерываний генерируемых процессором.
- прерывания
Обработка прерываний от перефирийных устройств.
...
Помимо определений процесса, которые были даны выше, можно рассматривать процесс как:
- программный код (readonly - может совместно использоваться другими процессами, которые выполняют ту же самую программу)
- набор данных, связанных с этим кодом

В любой конкретный момент времени выполнения программы процесс может быть уникально охарактеризован рядом элементов, включая следующие:
- идентификатор
Уникальный id процесса
- состояние
Будут рассмотрены ниже. Например, если процесс прямо сейчас исполняется на процессоре, то он находится в **состоянии выполнения**.
- приоритет
- программный счетчик
- указатели памяти
- данные контекста
...
Список файлов, которые использует процесс, запросы процесса на ввод-вывод и тд.
- учетная информация
Может включать количество используемого процессорного времени и тд.

Вся эта информация хранится в структуре данных, которая называется <u>**управляющим блоком процесса (PCB)**</u>. Она создается и управляется ОС. Важный факт про PCB заключатся в том, что он содержит всю информацию, которой достаточно, чтобы прервать исполнение текущего процесса и через время корректно возобновить его работу. PCB - ключевой инструмент ОС, который позволяет обеспечивать поддержку выполнения нескольких процессов.

Когда процесс прерывается, текущие значения регистров и в том числе счетчика команд сохраняются в PCB, а состояние процесса с "выполняется" меняется на некоторое другое состояние, например: "готов к выполнению" или "заблокирован". После этого операционная система диспетчеризует на процессор другой процесс. Когда ОС переводит в состояние выполнения другой процесс, из PCB восстанавливаются все регистры, а состояние меняется на - "выполняется".

Таким образом, можно сказать, что процесс состоит из
- программный код
- набор данных, связанных с этим кодом
...
В самом простейшем случае состояния должно быть как минимум два: "выполняется" и "не выполняется".
В таком случае ОС может работать так: по прерыванию от таймера раз в какое-то время (квантование времени) ОС может снимать процесс с процессора (сохранять регистры и менять состояние с "выполняется" на "не выполняется") и диспетчеризовать на процессор какой-нибудь другой процесс (восстанавливать регистры и менять состояние). А так же снимать процесс с процессора при выполнении операций ввода вывода.
Чтобы это организовать имеет смысл создать очередь, где будут храниться указатели на PCB процессов (ну или идентификаторы, но тогда ОС должна будет откуда-то из другого места брать PCB основываясь на ID но плевать на самом деле). И по очереди определять процессы на процессор, а когда очередной процесс будет снят с процессора, то класть его в конец очереди.
Создание нового процесса тогда будет осуществляться как создание процесса в состоянии "не выполняется" и помещение его в конец очереди.

![alt text](image-21.png)

### Модель с пятью состояниями

Модель с двумя состояниями могла бы работать эффективно, если бы все процессы всегда были бы готовы к выполнению. Но если например процесс снят с процессора из-за операции с ВУ, то это не так. Таким образом, может возникнуть ситуация, когда очередной процесс, стоящий во главе очереди будет все еще ожидать окончания операции ввода-вывода (или в общем случае просто какого-то события). И по идее ОС, должна пройтись по очереди и найти первый процесс, который готов выполняться. Но для этого придется добавить еще состояния.

![alt text](image-22.png)

А лучше вообще не класть все процессы в одну очередь, а сделать две очереди для "заблокированных" процессов (ожидающих событие) и "готовых к выполнению". Тогда для диспетчеризации на процессор ОС будет брать процессы из очереди "готовых к выполнению" процессов. А "заблокированные" процессы будет перемещать в очередь "готовых к выполнению" по мере наступления событий. А если еще немного подумать, то кажется, что чтобы ОС могла переместить процесс из "заблокированной" очереди, то ей придется мониторить все события и на каждое из них полностью сканировать очередь "заблокированных", чтобы понять, повляло ли текущее событие на разблокировку какого-то процесса. Поэтому лучше сделать много очередей на каждую блокировку.

![alt text](image-23.png)

Так же есть еще состояния "новый" и "завершающийся"

Новый - Только что созданный процесс, который еще не помещен операционной системой в пул выполнимых процессов. Обычно это новый процесс, код которого еще не загружен в основную память, а память под данные не выделена, хотя управляющий блок процесса уже создан.

Завершающийся - Процесс переходит в состояние завершающегося:
- при достижении точки естественного завершения
- когда он останавливается из-за возникновения неустранимой ошибки
- когда его останавливает другой процесс (SIGKILL), обладающий необходимыми для этого полномочиями

Операционная система временно сохраняет завершившийся процесс в памяти, пока другие программы (родительский процесс, программа, ведущая учет ресурсов ...) не извлекут всю необходимую информацию, после чего он полностью удаляется из системы.

### Модель с семью состояниями (приостановленные (suspended) процессы)

#### Swapping

Чтобы рассказать, что такое swapping, нужно на время забыть про виртуальную память, где механизмы swapping'а и paging'а и лежат в ее основе.

Если суммарная память, занимаемая всеми процессами, которые мы хотим запустить на компьютере превышает физическую, то приходится что-то выдумывать. В принципе, судя по истории развития ОС, самые примитивные операционные системы и мониторы уже умели выгружать процесс во вторичную память. Так что как будто это не какая-то гениальная мысль или нововведение. Итак, если процессов больше, чем памяти, а нам нужна многозадачность, то процессы нужно периодически выгружать на постоянную память. Логично выгружать заблокированные процессы. Но в любом случае понадобятся еще состояния, сигнализирующие о том, что процесс выгружен или не выгружен.

А ну в общем-то swapping - это выгрузка процесса на диск. А точнее, подразумевается, что процессов больше, чем памяти. Значит по определению, какое-то количество процессов уже находится во вторичной памяти. И когда процесс из основной выгружается во вторичную, по сути происходит замена процесса из оперативной памяти на какой-то процесс из постоянной памяти. Отсюда и название. Хотя, возможно, это не является ограничением. Я думаю, что ничего не мешает ОС просто выгрузить процесс во вторичную память, не заполняя его место другим процессом. Ну да, точно. Процессы ведь разного размера могут быть, поэтому 1 на 1 в любом случае может и не удастся свопнуть. А, ну да, точно можно просто выгрузить, ведь вдруг мы хотим выгрузить заблокированный процесс, потому что хотим создать новый. Тогда фактического swap (замены) не будет.

Когда речь идет о swapping'е, подразумевается именно выгрузка процесса целиком. А вот paging это уже выгрузка отдельных страниц на диск.

#### 

В общем кажется, что новая модель должна выглядеть как-то так:

![alt text](image-24.png)

Но на самом деле, скорее, так:

![alt text](image-25.png)

Ну вообще да, логично, что процесс как минимум может стать "готовым к выполнению", находясь в свопе, если пришло какое-то событие. Было бы странно, если бы ОС обязана была сразу же доставать из свопа процесс, который стал готовым к выполнению (ведь вообще не факт, что есть какой-то более "плохой" процесс на который его можно будет заменить. Все процессы в оперативке могут быть готовы к выполнению и иметь высокий приоритет например).

Но вообще тут еще есть некоторые неочевидные вещи: многие стрелочки могут вызвать вопросы. Например из состояния Новый процесс может перейти в Готовый/Приостановленный. Или из готового к выполнению в Готовый/Приостановленный. Или прям из выполняющегося в Готовый/Приостановленный. Но и вообще наверное может тоже показаться странным переход из Блокированный/Приостановленный в Блокированный (то есть получается ОС достала заблокированный поток из свопа, видимо освободилось место в памяти. Но зачем? Вдруг он будет вечно заблокированный).

- Готовый -> Готовый/Приостановленный
Если вдруг так случается, что процессов все таки больше чем памяти, а нужно как-то симулировать параллельность, то приходится делать так. Иногда даже готовый к выполнению процесс может стать лучшим кандидатом на выгрузку (если заблокированных просто нет, или есть, но у него супер высокий приоритет или что-то такое)

- Новый -> Готовый/Приостановленный
Суть в том, что может быть мы хотим создать новый процесс, когда основная память уже переполнена. Тогда просто создается pcb, создается образ процесса и пишется в своп (старая модель). То есть по факту процесс честно создается целиком и записывается в своп. В новой модели с виртуальной памятью, в таком случае в своп вообще ничего не пишется. То есть pcb создается, там уже записано логическое адресное пространство, но на диске по факту нет ничего. Сегмент данных (раз уж процесс новый) и сегмент кода можно будет создать сразу из исполняемого файла.

На самом деле страницы с кодом программы в целом никогда не выгружаются в своп, так как его всегда можно взять из исполняемого файла (Интересно, что если во время выполнения программы попытаться удалить исполняемый файл, то все равно все будет ок, так как по факту файл удаляется, только когда счетчик ссылок = 0, а в pcb указан дескриптор исполняемого файла, так что все все равно будет работать ок). 

- Выполняющийся -> Готовый/Приостановленный
Ну просто может так случиться что по истечению кванта времени выясняется что лучший кандидат на своп это процесс, который только что выполнялся, даже с учетом того, что он не был заблокирован.


Ну и кстати, то что процесс "Приостановлен" это не значит что он обязательно в свопе. Приостановлен = планировщик не имеет права его запускать. Можно например остановить пользовательский процесс командой:

```sh
kill -STOP pid
```

Но это не значит, что он помещается в своп.
