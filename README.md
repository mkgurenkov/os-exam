# 1-1. Архитектура компьютерных систем
...
Работа с компьютером происходило приблезительно так же, как и в БЭВМ. То есть был какой-то пульт управления и человек (оператор), который занимался управлением.

Оператор должен был:
- получить программу с данными от программиста
- подготовить программа к загрузке (например с перфокарт)
...
Память ЭВМ явно делилась под пользовательские программы (и данные) и системный монитор, который состоял из
...
Уже появился какой-то планировщик заданий. То есть, когда какая-то программа простаивала (занималась вводом-выводом например), могла выполняться другая пользовательская программа.

## Многозадачность

Вот тут иллюстрация проблемы простоя:

![alt text](image-6.png)

![alt text](image-7.png)

Но на самом деле надо понимать, что если "ожидание" для всех программ, это ожидание ввода-вывода для одного устройства, то это не будет так хорошо работать, так как устройство будет узким местом.

Но идея в целом рабочая.

## Системы с разделением времени

Суть в том, чтобы все-таки наконец уже исключить оператора и дать возможность одновременно нескольким пользователям пользоваться ЭВМ.

Тут уже появилась первая операционная система, которая позволяла делить ресурсы между пользователями (распределять процессорное время с помощью квантования в основном).

То есть к одному компьютеру подключили несколько терминалов и посадили пользователей за них. Они могли сами загружать программы и получать вывод на терминалы. Операционная система делила процессорное время между пользователями.

Но никакой виртуальной памяти еще не было, все программы грузились в основную память и в теории каждый пользователь мог все что угодно надергать из памяти (данные других пользователей). Но поскольку в то время чувствительные данные на компьютерах особо не обрабатывались, всем было плюс минус все равно.

Но конечно с точки зрения того, что одна плохо написанная программа могла испортить соседнюю - это проблема.

# 1-4. Основные задачи операционных систем

## Процесс

Первое упоминание понятия "процесс" появилось в операционной системе multics (предшественница unix).
...
Поток - единица потребления <u>процессора</u> у процесса.
...
Сегмент кода - набор машинных команд, представляющий скомпилированную программу (readonly). Интересный факт: так как код - readonly, иногда, если работают несколько одинаковых процессов, то сегмент кода может шариться между ними.

Сегмент данных - нельзя исполнять, можно писать и читать. Тоже интересный факт: если запущено несколько одинаковых процессов, то до того, как данные (из сегмента данных) какого-то процесса будут изменены, сегмент кода тоже может шариться между процессами. Вдруг данные никогда не будут изменены. 

Куча (heap) - память, выделяемая динамически.

Стек - в основном тут адреса возвратов и параметры, передаваемые в функцию. Интересные факт: вообще оффтоп, но в виртуальной машине джава, параметры для функции вообще не могут передаваться через регистры, только через стек.

TODO - почитать про статические и динамические библиотеки

Процесс включает в себя:
- исполняемая программа
- набор потоков исполнения
- связанные структуры ядра (абстракции, связанные с процессом, типа пулл потоков и тд)
- адресное пространство
    - код
    - данные
    - стек
    - куча
- контекст исполнения (слово многозначное: состоянии регистров может быть контекстом, и, то, работает ли процесс на уровне ядра или пользователя - тоже контекст)
- контекст безопасности (от имени какого пользователя, группы и тд. запущен процесс как я понял)
- ресурсы (скорее ссылки на ресурсы - абстракции, представляющие открытые файлы, сетевые соединения и тд)
- динамические библиотеки

### Проблемы современных процессов

- Защиты памяти процессов (ни один процесс не должен смочь испортить другой процесс)
...
Вот уже переходим непосредственно к задачам операционных систем. Первая из них это управление памятью (довольно сумбурно честно говоря):
- изоляция процессов
- управление выделением и освобождением памяти
- поддержка модулей (динамическая загрузка модулей, например драйвером)
- защита и контроль доступа (права на сегменты памяти: e.g. не исполнять сегмент данных)
- долговременное хранение
- страничный обмен (paging, swapping)

## Виртуальная память

Виртуальная память позволяет организовать отдельное адресное пространство для каждого процесса и ядра. То есть каждый процесс думает, что ему доступна вся память целиком, а механизмы виртуальной памяти уже делают так, чтобы это работало. 

TODO что значит для каждого ядра?

Другая функция виртуальной памяти, это предоставление возможности использовать больше памяти чем есть на самом деле. То есть например, виртуальное адресное пространство процесса может быть больше чем физическое адресное пространство. Ну или по размеру может быть таким же, как и физическое, но процессов может быть 2 или 10. Неважно, суть в том, что как будто бы памяти используется больше, чем есть на самом деле. Это работает, потому что процессам как правило не нужно иметь в памяти себя целиком в один момент времени, и какие-то страницы можно временно выгрузить на диск.

Ну или на самом деле можно не выгружать на диск а просто удалить, если например это readonly сегмент кода, ведь потом эту же страницу можно будет загрузить из исполняемого файла.

Управление MMU и TLB.

Некоторые страницы могут быть невыгружаемыми. Например, если страница используется многими процессами.

TODO про mmu и tlb почитать

## Защита информации и безопасность ОС

- доступ к системе (защита от несанкционированного доступа)
- конфиденциальность (невозможность неавторизованного доступа к данным. Тут про права, если у юзера нет прав на определенные данные, он не должен иметь к ним доступ)
- целостность данных (защита от неавторизованного и нецелостного изменения)
...
Планирование в операционной системе может быть осуществлено в соответствии с каким-то из следующих принципов:
- равноправие (пользователи (процессы) должны получать ресурсы равноправно)
- дифференциация отклика (как раз антиравноправие. Отклик некоторых приложений должен быть быстрее чем у других. Например, если пользователь прямо сейчас (окошко в фокусе) работает с каким-то интерактивным процессом, то ему следует повысить приоритет)
- общесистемная эффективность


# 1-5. Современные архитектурные концепции операционных систем

Ядро операционной системы связано с user space интерфейсом системных вызовов.

![alt text](image-9.png)

Само ядро условно делится на какое-то количество подсистем. Основные:
- управление памятью
- управление процессами и потоками
- файловая подсистема
- сетевая подсистема

\+ ядро включает в себя платформо-зависимый код (части операционной системы, использующие какие-то фичи процессора) и драйверы устройств.

## Архитектуры ядер

Важная оффтоп штука, потом подумаю, куда это вставить. Насколько я понял в процессе практически единственное что есть от ядра на самом деле это ядерный стек. По факту же код ядра, который выполняет процесс при сисколлах или при возникновении прерывания, это общий для всех процессов код, который лежит в физической памяти. Но ядро отображается в виртуальную память процесса, так что каждый процесс думает, что у него свое ядро. Но эти ядерные вертуальные страницы для всех процессов ведут в одно и то же место в физической памяти.

![alt text](image-10.png)

- Монолитное ядро. Сейчас обычно используется только во встроенных системах. Ничего динамически не подгрузить. Скомпилировали, загрузили в основную память, она работает. Если нужно что-то поменять, добавить какой-нибудь модуль, то придется перекомпилировать и заново загружать.
...
- Микроядро. Суть в том, чтобы в само ядро выделить какие-то самые базовые штуки типа управления процессами и памятью. А на других уровнях уже расположить другие подсистемы. Но по итогу оказалось что слишком частое переключение контекстов тормозит систему, и от этой идеи практически отказались.
...
- SMP. Симметричное мультипроцессирование. Когда все процессоры равны, нет какого-то центрального узла.
- ASMP. Ассиметричное мультипроцессирование. Отношение например как CPU - GPU.
...
# Раздел 7. Общая архитектура Unix/Linux

Есть определенные стандарт по которому определяется является ли ОС UNIX'ом или нет. Если ОС не является юниксом но реализует стандарт POSIX (стандартизует интерфейсы системных вызовов и библиотечных функций), то система является UNIX-Like.

И снова в основном общие моменты...

# Раздел 8. Общая архитектура Windows

Посмотрел, не очень интересно
